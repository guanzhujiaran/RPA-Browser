...
class BrowserDBService:
    ...
    async def create_fingerprint(
        params: UserBrowserInfoCreateParams, session: AsyncSession
    ) -> UserBrowserInfoCreateResp:
        """
        创建浏览器指纹信息
        """
        user_default_settings = await BrowserDBService.get_user_default_settings(
            params=UserBrowserInfoCountParams(browser_token=params.browser_token),
            session=session,
        )
        # 生成指纹数据
        ...
        browser_info = UserBrowserInfo(
            browser_token=params.browser_token, **fingerprint_data.model_dump()
        )
        ...
        await PluginDBService.get_or_create_user_default_plugins(
            browser_info.browser_token, session
        )
    ...
    async def read_fingerprint(
        params: UserBrowserInfoReadParams, session: AsyncSession
    ...
    async def update_fingerprint(
        params: UserBrowserInfoUpdateParams, session: AsyncSession
    ...
    async def delete_fingerprint(
        params: UserBrowserInfoDeleteParams, session: AsyncSession
    ) -> tuple[ResponseCode, bool, str]:
        stmt = select(UserBrowserInfo).where(
            and_(
                UserBrowserInfo.id == params.id,
    ...
    @staticmethod
    async def count_fingerprint(
        params: UserBrowserInfoCountParams, session: AsyncSession
    ) -> int:
    ...
    async def list_fingerprint(
        params: UserBrowserInfoListParams, session: AsyncSession
    ) -> BasePaginationResp[UserBrowserInfoReadResp]:
        cnt = await BrowserDBService.count_fingerprint(
            UserBrowserInfoCountParams(**params.model_dump()), session
        )
        if cnt == 0:
        ...
        return BasePaginationResp(total=cnt, items=res)
    ...
    async def get_user_default_settings(
        ...
        stmt = select(UserBrowserDefaultSetting).where(
            UserBrowserDefaultSetting.browser_token == params.browser_token,
        )
        result = await session.exec(stmt)
        res = result.one_or_none()
        return res