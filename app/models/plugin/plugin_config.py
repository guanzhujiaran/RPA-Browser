...
class RandomWaitPluginConfig(BaseModel):
    ...
    is_enabled: bool = True
    min_wait: float = 1.0  # 与 UserBrowserDefaultSetting.default_min_wait 保持一致
    mid_wait: float = 10.0
    max_wait: float = 30.0  # 与 UserBrowserDefaultSetting.default_max_wait 保持一致
...
DEFAULT_PLUGIN_CONFIGS = {
    "log": LogPluginConfig(),
    "page_limit": PageLimitPluginConfig(),
    "random_wait": RandomWaitPluginConfig(),
    "retry": RetryPluginConfig()
}


def get_default_plugin_config(plugin_type: str) -> BaseModel:
    """
    获取指定类型的默认插件配置
...
def is_plugin_config_changed(plugin_type: str, current_config: dict) -> bool:
    """
    检查插件配置是否与默认配置不同
...
def get_plugin_config_diff(plugin_type: str, current_config: dict) -> dict:
    """
    获取插件配置与默认配置的差异
