...
用于 HTTP 请求 Action 通过 RabbitMQ RPC 调用 FastapiApp 内部业务方法：
- 每个业务方法对应一个独立的 routing_key（= 队列名）
- 前端通过 API 获取此列表作为可选项展示，不允许随意输入外部接口
- 后端定时执行工作流时不依赖 JWT，直接通过 RPC 调用内部方法
...
# routing_key 前缀（与 FastapiApp 侧 controller/v1/mq/rpc_server.py 保持一致）
...
class RpcMethodName(StrEnum):
    ...
    `FastapiApp.rpc.<method_name>`。必须与 FastapiApp 侧
    controller/v1/mq/lottery_data.py 中 @rpc_subscriber 装饰的方法名一一对应。

    NONE 表示"不使用 RPC"（走 HTTP 模式），供前端下拉框作为空值选项，
    后端校验和执行时等价于 method_name=None。
    """

    # 空值：不使用 RPC，走 HTTP 模式（前端下拉框的"无"选项）
    NONE = ""
    # === LotteryData 核心接口 ===
...
# method_name 必须与 FastapiApp 侧 controller/v1/mq/lottery_data.py 的 @rpc_subscriber 保持一致
