...
class ExecutionRequest(SQLModel):
    """执行请求基础参数"""
    mid: int | str = Field(description="用户ID")
    ...
    execution_id: str = Field(
        default="",
        description="执行批次ID；为空时由执行引擎自动生成，定时调度场景由调度侧预生成以便关联运行记录",
    )
...
class ActionExecutionRequest(ExecutionRequest):
    ...
    params: Dict | AllActionParams | None = Field(default=None, description="操作参数")
...
def params_to_dict(params: AllActionParams | Dict | None) -> Dict:
